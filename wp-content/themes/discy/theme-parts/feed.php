<?php if (is_array($home_feed) && !empty($home_feed)) {
			...
			if ($key == "users" && isset($home_feed["users"]["value"]) && $home_feed["users"]["value"] == "users") {
				echo "<div class='feed-section feed-section-".$key."'>";
					...
					if ($users_more == "on") {
						...
						if (isset($pages) && isset($pages[0]) && isset($pages[0]->ID) && $count_all > $total_query) {
							echo "<div class='clearfix'></div><div class='load-more feed-show-more'><a href='".get_the_permalink($pages[0]->ID)."'>".esc_html__("More Users","discy")."</a></div>";
						}
					}
				echo "</div>";
				...
				echo "<div class='feed-section feed-section-".$key."'>
					...
					if (!empty($terms) && !is_wp_error($terms)) {
						...
						if ($cats_more == "on") {
							...
							if (isset($pages) && isset($pages[0]) && isset($pages[0]->ID) && count($terms_all) > count($terms)) {
								echo "<div class='clearfix'></div><div class='load-more feed-show-more'><a href='".get_the_permalink($pages[0]->ID)."'>".esc_html__("More Categories","discy")."</a></div>";
							}
						}
					}
				echo "</div>";
				...
				echo "<div class='feed-section feed-section-".$key."'>
					...
					if (!empty($terms) && !is_wp_error($terms)) {
						...
						if ($tags_more == "on") {
							...
							if (isset($pages) && isset($pages[0]) && isset($pages[0]->ID) && count($terms_all) > count($terms)) {
								echo "<div class='clearfix'></div><div class='load-more feed-show-more'><a href='".get_the_permalink($pages[0]->ID)."'>".esc_html__("More tags","discy")."</a></div>";
							}
						}
					}
				echo "</div>";
	...
	<div class='load-more finish-follow".($user_following_if == "yes" && $cat_following_if == "yes" && $tag_following_if == "yes" && ($all_following != "" || $user_following != "")?"":" not-finish-follow")."'>
		<div></div>
		<a href='' data-post='".$post_id_main."'>".($user_following_if == "yes" && $cat_following_if == "yes" && $tag_following_if == "yes" && ($all_following != "" || $user_following != "")?esc_html__("Click here to continue.","discy"):esc_html__("Complete your following to continue.","discy"))."</a>
	</div>";
}?>