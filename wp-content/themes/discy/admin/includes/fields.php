					...
					$output .= '<ul id="'.(isset($field_id) && $field_id != ""?$field_id:"").'-ul" class="sort-sections sort-sections-ul">';
						...
						if (isset($val) && is_array($val)) {
							foreach ($val as $key_a => $value_a) {
								$term_cat = get_term_by('id',$value_a,(isset($value['taxonomy']) && $value['taxonomy'] != ""?$value['taxonomy']:'question-category'));
								$i++;
								$output .= '<li class="li class="categories"" id="'.(isset($field_id) && $field_id != ""?$field_id:"").'_categories_'.$value_a.'"><div class="widget-head">
									<span>'.((isset($value['option_none']) && $value['option_none'] != "" && $value_a == $value['option_none']) || $value_a == "0"?esc_html__('All Categories','discy'):$term_cat->name).'</span></div><input name="'.(isset($field_id) && $field_id != ""?$field_id:"").'[cat-'.$value_a.']" value="'.$value_a.'" type="hidden">
									<div>
