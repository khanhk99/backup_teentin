...
function discy_admin_terms($tax = "",$term_id = "") {
	...
	if (isset($tax) && $tax == "question-category") {
		...
		$options[] = array(
			...
			'desc' => esc_html__("Select 'On' to enable new category. (In a new category, the admin must answer the question before anyone else and the user has ask question and admin only can answer).","discy"),
			'id'   => prefix_terms.'new',
			'type' => 'checkbox'
		);
