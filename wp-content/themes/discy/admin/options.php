...
function discy_admin_options() {
	...
	if (class_exists("WPQA")) {
		...
		*/
		...
		if (class_exists("WPQA")) {
			$options[] = array(
				'name' => '<a href="'.wpqa_add_question_permalink().'" target="_blank">'.esc_html__('The Link For The Ask Question Page.','discy').'</a>',
				'type' => 'info'
			);
		}
		...
		$payment_methodes = array(
			"paypal" => array("sort" => esc_html__('PayPal','discy'),"value" => "paypal"),
			"stripe" => array("sort" => esc_html__('Stripe','discy'),"value" => "stripe"),
		);
		
		$options[] = array(
			'name'    => esc_html__('Select the payment methodes','discy'),
			'id'      => 'payment_methodes',
			'type'    => 'multicheck',
			'sort'    => 'yes',
			'std'     => $payment_methodes,
			'options' => $payment_methodes,
		);
		...
		$options[] = array(
			...
			'options' => 
				array(
					"payments" => esc_html__('Payment methodes','discy'),
		...
		$options[] = array(
			'name'    => esc_html__('Payment way','discy'),
			'desc'    => esc_html__('Choose the payment way sticky the question','discy'),
		...
		$options[] = array(
			'name' => esc_html__('Subscriptions','discy'),
			'desc' => esc_html__('Select ON to active subscriptions.','discy'),
		...
		if (class_exists("WPQA") && wpqa_plugin_version >= 3.9) {
			$options[] = array(
		...
		$options[] = array(
			'name' => '<a href="https://2code.info/docs/discy/subscriptions/" target="_blank">'.esc_html__('To make the paid subscriptions work well, check this link.','discy').'</a>',
			'type' => 'info'
		);

		$options[] = array(
			'name'    => esc_html__('Paid group for the subscriptions','discy'),
			'desc'    => esc_html__('Select the paid group for the subscriptions','discy'),
		...
		$options[] = array(
			...
			'options' => array(
				...
				"3months" => esc_html__('3 Months','discy'),
				"6months" => esc_html__('6 Months','discy'),
				"yearly"  => esc_html__('Yearly','discy'),
			)
		);

		$options[] = array(
			"name"      => esc_html__("What's price to subscribe monthly?",'discy'),
		...
		$options[] = array(
			"name"      => esc_html__("What's price to subscribe 3 months?",'discy'),
		...
		$options[] = array(
			"name"      => esc_html__("What's price to subscribe 6 months?",'discy'),
		...
		$options[] = array(
			"name"      => esc_html__("What's price to subscribe yearly?",'discy'),
			"id"        => "subscribe_yearly",
			"type"      => "text",
	...
	$options[] = array(
		...
		'desc' => esc_html__('Select ON if you want to make the login works without ajax to avid the problems with cache plugin.','discy'),
		'id'   => 'stop_login_ajax',
		'type' => 'checkbox'
	);
	...
	$edit_profile_sections = array(
		...
		array('name' => esc_html__('Mail settings','discy'),"value" => "mails_setting","default" => "yes"),
	...
	$options[] = array(
		'name'      => esc_html__('Note: if you need this mail template, From Discy Settings >> Question settings >> Questions category settings >> Activate the users to request a new category && Send mail when the category need a review.','discy'),
		'operator'  => 'or',
	...
	$options[] = array(
		'id'   => 'uniqid_cookie',
		'std'  => (class_exists("WPQA")?wpqa_token(15):rand(1,100000)),
		'type' => 'hidden'
	);
