...
define("discy_old_version",get_option("discy_old_version"));
	...
	function discy_enqueue_scripts( $hook ) {
		...
		if ( $enqueue ) {
			...
			$discy_js = array(
				...
				"without_ads"               => esc_html__("Select ON to remove the ads.","discy"),
			);
                    	...
                    	<div class="comment-author">
                    		...
                    		if ($wpqa_activate_comment_author == true) {
                    			if ($comment_user_id > 0) {
	                    			...
	                    			$get_author_profile = ($comment->comment_author_url != ""?$comment->comment_author_url:"discy_No_site");
	                    		}
		...
		if (($custom_permission == "on" && is_user_logged_in() && !is_super_admin($user_id) && empty($roles["without_ads"])) || ($custom_permission == "on" && !is_user_logged_in())) {
			if ($class == "discy-ad-header") {
				...
				if ($subscriptions_payment == 'on') {
					$show_alert = '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Do you need to remove the ads?","wpqa").' '.wpqa_paid_subscriptions().'</p></div>';
				}
			}
		}else {
			$show_ads = false;
		...
		$output .= '<div class="'.$class.'">';
			if ($adv_type == "custom_image") {
				if ($adv_href != "") {$output .= '<a href="'.esc_url($adv_href).'">';}
					$output .= '<img alt="'.esc_attr_e("Adv","discy").'" src="'.esc_url($adv_img).'">';
