...
function discy_term_options( $args ) {
	if (is_array($args) && !empty($args)) {
		foreach ($args as $taxonomy) {
			add_action( $taxonomy .'_add_form_fields', 'discy_term_add_form_fields' );
			add_action( $taxonomy .'_edit_form_fields', 'discy_term_edit_form_fields', 10, 2 );
			add_action( 'edited_'. $taxonomy, 'discy_save_term', 10, 2 );
			add_action( 'create_'. $taxonomy, 'discy_save_term', 10, 2 );
		}
	}
}
