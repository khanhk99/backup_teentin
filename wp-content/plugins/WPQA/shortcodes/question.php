				...
				foreach ($question_sort as $sort_key => $sort_value) {
					...
					if ($sort_key == "title_question" && ((isset($question_sort["title_question"]["value"]) && $question_sort["title_question"]["value"] == "title_question") || (isset($question_sort["comment_question"]["value"]) && $question_sort["comment_question"]["value"] != "comment_question"))) {
						...
						$out .= ($suggest_questions == "on"?'<div class="the-title-div">':'<p>').
							...
							<input name="title" id="question-title-'.$rand.'" class="the-title'.($suggest_questions == "on"?" suggest-questions live-search live-search-icon":"").'" type="text" value="'.($type == "add" && isset($question_add['title'])?wpqa_kses_stip($question_add['title']):($type == "edit"?(isset($question_edit['title'])?wpqa_kses_stip($question_edit['title']):wpqa_kses_stip($get_post_q->post_title)):"")).'"'.apply_filters("wpqa_question_title_attrs",false).'>
							<i class="icon-chat"></i>
	...
	if ($form_type == "add_question") {
		...
		if (($custom_permission != "on" && ((isset($user_login_group) && $user_login_group == "wpqa_under_review") || (isset($user_login_group) && $user_login_group == "activation"))) || ($custom_permission == "on" && (is_user_logged_in() && !is_super_admin($user_id) && empty($roles["ask_question"])) || (!is_user_logged_in() && $ask_question != "on"))) {
			...
			if (!is_user_logged_in() && $pay_ask == "on") {
				...
				if (!wpqa_check_if_user_subscribe($user_id) && isset($_allow_to_ask) && (int)$_allow_to_ask < 1 && $pay_ask == "on" && $payment_group[$user_login_group] != $user_login_group) {
...
function wpqa_question($atts, $content = null) {
	...
	if (($custom_permission != "on" && ((isset($user_login_group) && $user_login_group == "wpqa_under_review") || (isset($user_login_group) && $user_login_group == "activation"))) || ($custom_permission == "on" && (is_user_logged_in() && !is_super_admin($user_get_current_user_id) && empty($roles["ask_question"])) || (!is_user_logged_in() && $ask_question != "on"))) {
		...
		if (!is_user_logged_in() && $pay_ask == "on") {
			...
			if (!wpqa_check_if_user_subscribe($user_get_current_user_id)) {
			...
			if (!wpqa_check_if_user_subscribe($user_get_current_user_id) && isset($_allow_to_ask) && (int)$_allow_to_ask < 1 && $pay_ask == "on" && $payment_group[$user_login_group] != $user_login_group) {
		...
		if ($edit_question == "on" || $is_super_admin || (isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit")) {
			...
			if (isset($get_post) && $get_post != 0 && $get_question && $get_question->post_type == "question") {
				if ($get_question->post_author != 0 || $is_super_admin || (isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit")) {
					...
					if ($allow_to_edit_question == true && (($get_question->post_author == $user_id && $user_id != 0 && $get_question->post_status == "publish") || $is_super_admin || (isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit"))) {
				...
				$out .= '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry no question has you select or not found.","wpqa").'</p></div>';
			}
		}else {
...
function wpqa_process_edit_questions($user = "") {
	...
	if ($form_type == "edit_question") {
		...
		$fields = array(
			'title','comment','category','question_poll','question_image_poll','remember_answer','question_tags','video_type','video_id','video_description','sticky','ask','private_question','anonymously_question','featured_image','publish_question','pending'
		);
