(function($) { "use strict";
	...
	if (jQuery(".review-post").length) {
		jQuery(".review-post,.a-pending-post,.d-pending-post,.b-pending-post,.u-pending-post").live("click",function () {
			...
			if (pending_post.hasClass("review-post")) {
				...
				if ((pending_post.hasClass("b-pending-post") && confirm(wpqa_js.sure_ban)) || pending_post.hasClass("u-pending-post") || (pending_post.hasClass("d-pending-post") && pending_post.hasClass("delete-pending-question") && confirm(wpqa_js.sure_delete)) || (pending_post.hasClass("d-pending-post") && pending_post.hasClass("delete-pending-post") && confirm(wpqa_js.sure_delete_post)) || pending_post.hasClass("a-pending-post")) {
						...
						success:function(data) {
							if (pending_type == "ban" || pending_type == "unban") {
								if (pending_type == "ban") {
									...
									pending_post.removeClass("b-pending-post").addClass("u-pending-post").find("span").text(wpqa_js.unban_user);
								}else {
									pending_post.find("i").removeClass("icon-back").addClass("icon-cancel-circled");
									pending_post.removeClass("u-pending-post").addClass("b-pending-post").find("span").text(wpqa_js.ban_user);
								}
								...
								if (all_section.find(".article-post").length == 0) {
									all_section.append("<div class='alert-message warning'><i class='icon-flag'></i><p>"+(jQuery("#section-pending-questions").length > 0?wpqa_js.no_questions:wpqa_js.no_posts)+"</p></div>")
								}
							}
						}
					});
	...
	if (jQuery("#respond").length) {
		...
		jQuery("#respond #submit").live("click",function () {
			...
			if (wpqa_js.terms_active_comment == "on") {
				var agree_terms = jQuery("#respond #agree_terms_comment").is(":checked");
				if (agree_terms != 1) {
					jQuery("#respond .wpqa_error").animate({opacity: 'hide' , height: 'hide'}, 400).remove();
					jQuery("#respond .comment-form").prepend('<div class="wpqa_error">'+wpqa_js.wpqa_error_terms+'</div>');
	...
	if (jQuery(".payment-methods a").length) {
		jQuery(".payment-methods a").each(function () {
			jQuery(this).live("click",function () {
				var payment = jQuery(this);
				if (payment.hasClass("button-default-2")) {
					var payment_hide = payment.attr("href");
					payment.parent().find("a").addClass("button-default-2").removeClass("button-default-3");
					payment.addClass("button-default-3").removeClass("button-default-2");
					payment.parent().find(".payment-method").hide(10);
					payment.parent().find(".payment-method[data-hide="+payment_hide+"]").slideDown(300);
				}
				return false;
			});
		});
	...
	if (jQuery(".subscribe-section").length) {
		if (window.location.hash == "#subscribe-monthly" || window.location.hash == "#subscribe-3months" || window.location.hash == "#subscribe-6months" || window.location.hash == "#subscribe-yearly" || window.location.hash == "#subscribe-lifetime") {
	...
	if (jQuery(".wpqa-stripe-payment").length && wpqa_js.publishable_key != "") {
		var stripe = Stripe(wpqa_js.publishable_key);
		...
		function confirm_card_payment($form, card, data) {
			stripe.confirmCardPayment(data.client_secret).then(function (result) {
				if (result.error) {
					jQuery('.wpqa_error', $form).text(result.error).animate({opacity: 'show' , height: 'show'}, 400).delay(5000).animate({opacity: 'hide' , height: 'hide'}, 400);
				}else {
					jQuery('input[name="payment-intent-id"]', $form).remove();
