...
if (!function_exists('wpqa_show_notifications')) :
	function wpqa_show_notifications($whats_type_result,$show_date = "") {
		...
		if ((((isset($whats_type_result["comment_id"]) && $whats_type_result["comment_id"] != "") || $whats_type_result["text"] == "add_question_user" || $whats_type_result["text"] == "add_question" || $whats_type_result["text"] == "poll_question") && empty($whats_type_result["username"]) && isset($whats_type_result["another_user_id"]) && $whats_type_result["another_user_id"] == 0) || (!empty($whats_type_result["another_user_id"]) || !empty($whats_type_result["username"])) && $whats_type_result["text"] != "admin_add_points" && $whats_type_result["text"] != "admin_remove_points") {
			
			if ((((isset($whats_type_result["comment_id"]) && $whats_type_result["comment_id"] != "") || $whats_type_result["text"] == "add_question_user" || $whats_type_result["text"] == "add_question" || $whats_type_result["text"] == "poll_question") && isset($whats_type_result["another_user_id"]) && $whats_type_result["another_user_id"] == 0) || (isset($display_name) && $display_name != "")) {
				if (!empty($whats_type_result["another_user_id"])) {
				...
				if (((isset($whats_type_result["comment_id"]) && $whats_type_result["comment_id"] != "") || $whats_type_result["text"] == "add_question_user" || $whats_type_result["text"] == "add_question") && empty($whats_type_result["username"]) && isset($whats_type_result["another_user_id"]) && $whats_type_result["another_user_id"] == 0) {
								...
								if ((isset($credential) && $credential != "") || (isset($city) && $city != "") || (isset($country) && $country != "") || (isset($phone) && $phone != "") || (isset($url) && $url != "") || (isset($gender) && $gender != "") || (isset($age) && $age != "")) {
									$out .= '<div class="user-data">
										<ul>';
											...
											if (isset($phone) && $phone != "" && $show_phone_profile == true) {
...
if (!function_exists('wpqa_message_button')) :
	function wpqa_message_button($author_id,$text = "",$owner = "",$return = "") {
		...
		if ($active_message == "on" && $owner == false) {
			...
			if (((!is_user_logged_in() && $send_message_no_register == "on") || (is_user_logged_in() && (empty($user_block_message) || (isset($user_block_message) && is_array($user_block_message) && !in_array($get_current_user_id,$user_block_message))) && ($block_message != "on" || is_super_admin($get_current_user_id)) && $received_message == "on"))) {
...
if (!function_exists('wpqa_count_posts_by_user')) :
	function wpqa_count_posts_by_user( $user_id, $post_type = "post", $post_status = "publish", $category = 0 ) {
		...
		$meta_query = ($post_type == "post"?array():array("meta_query" => array("relation" => "OR",array("key" => "private_question","compare" => "NOT EXISTS"),array("key" => "private_question","compare" => "=","value" => 0))));
		$args = array(
			"post_type"   => $post_type,
			"post_status" => $post_status,
		);
		$the_query = new WP_Query(array_merge($author,$tax,$meta_query,$args));
		return $the_query->found_posts;
		wp_reset_postdata();
	}
endif;
			...
			<span class="crumbs">
				<span itemscope itemtype="https://schema.org/BreadcrumbList">
				    ...
				    if (isset($text) && $text != "") {
				    	...
				    	echo ($before . esc_html__('Subscriptions', 'wpqa') . $after);
				        ...
				        if ($post_type != 'post') {
				        	if ($post_type == 'question') {
				        		...
				        		echo ($before_schema.wpqa_breadcrumbs_schema(esc_url(home_url('/').(isset($post_type->has_archive)?$post_type->has_archive:$slug['slug'])).'/',$post_type->labels->singular_name,2,'yes')).
	...
	function wpqa_head_content($login = "",$its_not_login = false) {
		...
		<div class="put-wrap-pop">
			<?php if (!is_user_logged_in()) {
				...
				if ($activate_need_to_login == "on" && !is_home() && !is_front_page() && (is_page() || is_single()) && isset($_COOKIE[wpqa_options("uniqid_cookie").'wpqa_locked_content'])) {
					$show_pop_login = true;
				}
				...
				<div class="pop-border-radius">
					...
					if ($its_not_login != true && $signup_style == "style_2") {?>
						<div class="panel-image-content">
							...
							<?php echo ' <a href="'.wpqa_login_permalink().'" class="login-panel'.apply_filters('wpqa_pop_up_class_login','').' button-default">'.esc_html__( 'Have an account?', 'wpqa' ).' '.esc_html__( 'Sign In', 'wpqa' ).'</a>';?>
				...
				<div class="pop-border-radius">
					...
					if ($its_not_login != true && $login_style == "style_2") {?>
						<div class="panel-image-content">
							...
							<?php echo ' <a href="'.wpqa_signup_permalink().'" class="signup-panel'.apply_filters('wpqa_pop_up_class_signup','').' button-default">'.esc_html__( 'Sign Up Here', 'wpqa' ).'</a>';?>
						</div><!-- End panel-image-content -->
					<?php }?>
					<div class="panel-pop-content">
		...
		if ($confirm_email != "yes" && $login != "login") {
			...
			if ($active_message == "on" && (wpqa_is_user_profile() || $active_message_filter == true)) {
				...
				if (((!is_user_logged_in() && $send_message_no_register == "on") || (is_user_logged_in() && (empty($user_block_message) || (isset($user_block_message) && is_array($user_block_message) && !in_array(get_current_user_id(),$user_block_message))) && ($block_message != "on" || is_super_admin($wpqa_user_id)) && isset($received_message) && $received_message == "on"))) {
					$custom_permission = wpqa_options("custom_permission");
...
/* Search load more */
add_filter("get_pagenum_link","wpqa_get_pagenum_link",1,2);
function wpqa_get_pagenum_link($result,$pagenum) {
	if (wpqa_is_search()) {
		global $wp_rewrite;
...
if (!function_exists('wpqa_pagination_load')) :
	function wpqa_pagination_load($post_pagination = "pagination",$max_num_pages = "",$it_answer_pagination = false,$its_question = false,$wpqa_query = null,$it_comment_pagination = false,$args = array()) {
