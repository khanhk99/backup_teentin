...
/* Get plugin vertion */
add_action('wpqa_init','wpqa_plugin_version');
function wpqa_plugin_version() {
	if (is_admin()) {
		global $wpqa;
		$get_plugin_version = get_option("wpqa_plugin_version");
		$wpqa_old_version = get_option("wpqa_old_version");
		if ($get_plugin_version == "") {
			add_option("wpqa_old_version",$wpqa->plugin_version());
		}
		if (isset($get_plugin_version) && $get_plugin_version != "") {
			if ($get_plugin_version != $wpqa->plugin_version()) {
				update_option("wpqa_old_version",$get_plugin_version);
				update_option("wpqa_plugin_version",$wpqa->plugin_version());
			}
		}else {
			add_option("wpqa_plugin_version",$wpqa->plugin_version());
		}
	}
}
/* Action author */
