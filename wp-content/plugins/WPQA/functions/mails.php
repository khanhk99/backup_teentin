...
if (!function_exists('wpqa_send_email')) :
	function wpqa_send_email($content,$user_id = 0,$post_id = 0,$comment_id = 0,$reset_password = "",$confirm_link_email = "",$item_price = "",$item_currency = "",$payer_email = "",$first_name = "",$last_name = "",$item_transaction = "",$date = "",$time = "",$category = "",$custom = "",$sender_user_id = "",$recieved_user_id = 0,$invitation_link = "") {
		...
		if (isset($first_name) && $first_name != "") {
			...
			$content = str_ireplace('[%first_name%]', ucfirst($user->display_name), $content);
		}else {
															...
															<div style="border:solid 1px #d9d9d9;padding-right:30px;padding-left:30px">
																<table style="line-height:1.6;font-size:12px;font-family:Helvetica,Arial,sans-serif;border:solid 1px #FFF;color:#444;" border="0" width="100%" cellspacing="0" cellpadding="0" bgcolor="#ffffff">
																	<tbody>
																		...
																		'.($schedule != ''?(isset($schedule_content) && $schedule_content != ''?wpqa_send_email($schedule_content):''):'').'
																	</tbody>
																</table>
															</div>
														</td>
...
function wpqa_action_schedule_mails() {
	$discy_schedules_time = get_option("discy_schedules_time");
	...
	if ($discy_schedules_time == "") {
		...
		$discy_schedules_time = ($schedules_time_day != "" && $schedules_time_hour != ""?strtotime(date("Y").'-'.date("m")." next ".$schedules_time_day." ".$schedules_time_hour.":00"):time());
		update_option("discy_schedules_time",$discy_schedules_time);
	}
	
	if (!wp_next_scheduled('wpqa_scheduled_mails_daily')) {
		wp_schedule_event($discy_schedules_time,'daily','wpqa_scheduled_mails_daily');
	}
	if (!wp_next_scheduled('wpqa_scheduled_mails_weekly')) {
		wp_schedule_event($discy_schedules_time,'weekly','wpqa_scheduled_mails_weekly');
	}
	if (!wp_next_scheduled('wpqa_scheduled_mails_monthly')) {
		wp_schedule_event($discy_schedules_time,'monthly','wpqa_scheduled_mails_monthly');
	}
}
/* Daily mails */
